...
const getErrorMessageByStatus = (status: number): string => {
  if (status === 400) {
    return 'No se pudo autenticar con Google. Intenta de nuevo.';
  }
...
export const GoogleCallbackPage = () => {
  ...
  useEffect(() => {
    ...
    const resolveGoogleCallback = async () => {
      ...
      try {
        ...
        setError('No fue posible completar la autenticacion con Google.');
      }
    };

    void resolveGoogleCallback();
  ...
  return (
    <div className="flex min-h-screen items-center justify-center px-4 py-10">
      <section className="w-full max-w-xl rounded-3xl border border-border bg-white p-8 shadow-md">
        ...
        {!error && <p className="mt-4 text-sm text-text-secondary">Validando credenciales de Google contra el backend...</p>}
