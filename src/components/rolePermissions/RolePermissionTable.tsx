...
interface RolePermissionTableProps {
  data: RolePermission[];
  loading: boolean;
  onEdit: (item: RolePermission) => void;
  onDelete: (item: RolePermission) => void;
}

export const RolePermissionTable = ({ data, loading, onEdit, onDelete }: RolePermissionTableProps) => {
  const columns: TableColumn<RolePermission>[] = [
    { key: 'roleId', header: 'Role ID', render: (item) => item.role.id },
    { key: 'permissionId', header: 'Permission ID', render: (item) => item.permission.id },
    {
      key: 'actions',
      header: 'Acciones',
  ...
  return (
    <Table
      columns={columns}
      data={data}
      emptyMessage="No hay relaciones role-permission registradas."
      rowKey={(row) => row.id}
    />
  );
};
