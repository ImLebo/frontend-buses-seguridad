...
export const ProfileTable = ({ data, loading, onEdit, onDelete }: ProfileTableProps) => {
  const columns: TableColumn<Profile>[] = [
    { key: 'phone', header: 'Telefono' },
    { key: 'photo', header: 'Foto URL' },
    {
      key: 'actions',
      header: 'Acciones',
  ...
  if (loading) {
    return <div className="rounded-lg border border-slate-200 p-4 text-sm text-slate-500">Cargando profiles...</div>;
  }

  return <Table columns={columns} data={data} emptyMessage="No hay profiles registrados." rowKey={(row) => row.id} />;
};
